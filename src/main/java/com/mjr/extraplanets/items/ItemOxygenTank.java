...
public class ItemOxygenTank extends Item implements ISortableItem {
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn){
		if (playerIn instanceof EntityPlayerMP) {
			...
			if (gear == null) {
				stats.getExtendedInventory().setInventorySlotContents(2, playerIn.getHeldItem(handIn).copy());
				playerIn.getHeldItem(handIn).setCount(0);
			} else if (gear1 == null) {
				stats.getExtendedInventory().setInventorySlotContents(3, playerIn.getHeldItem(handIn).copy());
				playerIn.getHeldItem(handIn).setCount(0);;
			}
		}
